...
func TestEffectivePolicy_ChunkedUploadMetaFailureDeniesNativeBaselineWithoutError(t *testing.T) {
	svc, _, _, _ := newTestService(t)
	...
	assert.Equal(t, false, checked[role.PolicyCanUseChunkedUpload])
	...
	assert.Equal(t, false, unchecked[role.PolicyCanUseChunkedUpload])
	assert.Equal(t, 20, unchecked["maxFileSizeMb"])
}

func TestEffectivePolicy_ChunkedUploadMetaFailureOverridesOrderedProviders(t *testing.T) {
	svc, _, _, _ := newTestService(t)
	keys := []string{
	...
	checked, err := svc.GetUserPoliciesChecked("u1")
	require.NoError(t, err)
	assert.Equal(t, false, checked[role.PolicyCanUseChunkedUpload], "the final cap boundary overrides provider priority and order")
