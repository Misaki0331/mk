...
func (s *Service) StartChunkedUpload(_ context.Context, in StartChunkedUploadInput) (*model.ChunkedUploadSession, error) {
	...
	// (#2611)。
	if maxBytes, enabled := policyMegabytes(policies["maxFileSizeMb"]); enabled {
		if in.Size > maxBytes {
	...
	if capacityBytes, enabled := policyMegabytes(policies["driveCapacityMb"]); enabled {
	...
	if maxPending, enabled := policyMegabytes(policies[role.PolicyChunkedUploadMaxPendingMb]); enabled {
